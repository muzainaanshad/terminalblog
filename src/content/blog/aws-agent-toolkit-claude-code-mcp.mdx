...
**Can I use the AWS MCP Server with agents other than Claude Code, Codex, and Cursor?**
Yes. Kiro connects directly without a plugin. Any MCP-capable agent can point at the AWS MCP Server manually by configuring the server endpoint. The server speaks standard MCP, so compatibility is broad.

**Does the AWS MCP Server give agents full shell access to my machine?**
No. The sandboxed Python execution environment is isolated from your host machine. Agents cannot run arbitrary shell commands — they execute Python in a controlled sandbox with the IAM permissions you grant. CloudTrail logs every API call for audit visibility.
