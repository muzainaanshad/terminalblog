...
Kiro Crew is an orchestration platform, not a coding agent. It coordinates multiple coding agents (Claude Code, Codex, Kiro CLI, etc.) across sessions, provides persistent memory, schedules recurring work, and gates operations behind human approval workflows. The agents do the coding; Kiro Crew manages the team.

**Can I use Kiro Crew with agents other than the Kiro CLI?**
Kiro Crew is built on open protocols (ACP and MCP), so it's designed to interoperate. However, at launch it runs on the proprietary Kiro CLI which is metered by credits. Using other agents (Claude Code, Codex) requires building and validating your own connectors — the "open part stops at the orchestration layer" until community connectors mature.

**Does Kiro Crew require an AWS account?**
No. Kiro Crew runs entirely inside your environment — laptops, containers, or VMs — with no AWS account and no AWS-managed control plane required. Code and credentials stay inside your perimeter.

**What are the strongest use cases for Kiro Crew?**
Platform engineering, DevOps, and SRE work — repetitive, long-running tasks like dependency upgrades, framework migrations, flaky test cleanup, first-pass incident investigation, and ticket triage. These benefit from async execution with checkpoints and retries.

**Is Kiro Crew ready for production use?**
It's free, open source, and self-hostable with 39,000+ internal Amazon engineers as a track record. But treat it as a pilot first: stand it up in dev, wire one real workflow, and measure whether it reduces human babysitting and whether governance/audit features hold under real least-privilege pressure.
