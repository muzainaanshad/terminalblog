...
GitHub Copilot has a generous API for authenticated users. It's surprising that more coding agents haven't integrated it as a provider. Hermes supports OpenAI-compatible endpoints, which might cover Copilot. Oh My Pi has Baseten, which is a different approach to open models. But no other agent has a dedicated Copilot provider.
