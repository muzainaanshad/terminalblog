...
This security hardening aligns with our [coding agent security checklist 2026](/blog/coding-agent-security-checklist-2026/) — the principles are the same across agents.
...
## v0.21.4 Patch Release — September 21, 2026

Three weeks after Pantheon launched, v0.21.4 (tagged `v2026.9.21`) rolls up the ~1,800 PRs merged since v0.21.3 into a stable tagged release for downstream consumers (Docker images, Hermes Cloud, hosted deployments). Full curated notes for this window are deferred to v0.22.0.

Measured at commit `4b8a813`, the window since v0.21.3 contains **5,071 non-merge commits** across **5,169 changed files** (+312,961 / −62,855), **1,812 merged PRs** and **2,116 closed issues**.

Highlights in this patch window (full notes ship with v0.22.0):

- **Host-wide gateway singleton lock** with a rendezvous record — Desktop now attaches to the running host backend instead of spawning a second one
...
- **Desktop chat/UI font picker**, one-click local engine updates, and plugin uninstall from the Plugins hub
- **`decline` unauthorized-DM behavior** for the gateway
- **Configurable MCP discovery connect cap** (`mcp.discovery_concurrency`)
- **`session_search` after/before bounds** plus OR-relaxed recall retry
- **`hermes sessions set-journal-mode`** command
...
- A dozen new community plugins in the catalog (tailscale, ssh, shodan, terminal, rss, resetwatch, done-bell, kiwi, cognee, Octen)
- Large run of profile/multiplex isolation, cron, kanban, Desktop and state.db fixes

**Updating:** `hermes update` (git installs), or re-run the installer one-liner. Docker / Hermes Cloud: images build from this tag (`nousresearch/hermes-agent:v2026.9.21`).

**Full changelog:** https://github.com/NousResearch/hermes-agent/compare/v2026.9.14...v2026.9.21

## v0.21.5 Patch Release — September 24, 2026

Hot on the heels of v0.21.4, **v0.21.5** (tagged `v2026.9.24`) drops just three days later — rolling up the ~460 PRs merged since v0.21.4 into another stable tagged release for downstream consumers (Docker images, Hermes Cloud, hosted deployments). The pace is relentless: this project does not sit still.

What this means for you: if you run Hermes in production (Docker, Hermes Cloud, or self-hosted gateway), update to `v2026.9.24` for the latest stability and fix wave. Full curated notes for the v0.21.4→v0.21.5 window are deferred to v0.22.0, but the tag exists so downstream deployments stay current.

**Updating:** `hermes update` (git installs), or re-run the installer one-liner. Docker / Hermes Cloud: images build from this tag (`nousresearch/hermes-agent:v2026.9.24`).
