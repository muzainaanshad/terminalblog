...
**Q: If AI models are getting better (SWE-bench 78-88%), why is developer trust falling (29%)?**
Trust fell because exposure grew faster than capability. When tools were new, developers used them on safe, scoped tasks (boilerplate, tests) where success rates were high. As usage expanded to multi-file refactors, autonomous debugging, and production changes, failure modes became visible. The gap between demo performance and 2 AM production reality is where trust erosion lives.

**Q: Why do senior engineers prefer Claude Code (46% "most loved") over Copilot (9%) despite Copilot's 4.7M subscribers?**
Seniors trust tools that make their judgment process transparent. Claude Code's step-by-step reasoning lets them inspect and verify before accepting. Copilot obscures reasoning — optimized for line-level autocomplete, not complex multi-file tasks. Enterprise adoption (Copilot 56% at 10K+ companies) is driven by procurement compatibility and risk floor, not peak capability.

**Q: Is using 2-4 AI coding tools simultaneously (70% of engineers) tool-hoarding or a rational strategy?**
It's risk distribution — a trust-compensation strategy. Cursor for daily editing (small, reviewable diffs), Claude Code for complex multi-file tasks (inspectable reasoning), Copilot for line-level autocomplete (two-second evaluation). Each tool covers a different trust threshold. The multi-tool stack is the rational response to no single tool earning full trust across all task types.

**Q: What's the real cost of "free" open-source agents like Cline, OpenHands, Aider?**
Near-zero subscription cost, but 2-3× more attempts per correct fix and lower resolution rates on complex tasks (per 1,200-issue benchmark). The per-issue cost is low; the per-verified-issue cost is not. Teams should measure time-to-verified-merge, not time-to-first-diff.

**Q: How should teams update their review workflows for AI-generated code?**
Sonar found reviewing AI code is now a "major time sink." The fix: AI-assisted review of AI-generated code, plus clear policies on which change categories require human review by default. Adopting agents without investing in review tooling is like deploying to production without a rollback strategy.

**Q: Will the trust gap close naturally as models improve, or is verification tooling the bottleneck?**
Verification tooling is the bottleneck. Model quality has never been higher (Claude Code 78-88% SWE-bench, GPT-5.3-Codex 85%), yet trust dropped from 40% to 29%. The ACADSI integration quality index declined from 48.4% to 39.6% YoY while raw adoption grew. Trust will come from verification systems that make output inspectable, testable, and rollbackable at generation speed — not from better models alone.
...
- [The State of Open-Source Coding Agents in 2026](/blog/state-of-open-source-coding-agents-2026/)