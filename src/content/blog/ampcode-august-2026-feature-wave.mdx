...
**What are AmpCode's Global Plugins and how do they differ from project-level skills?**
Global Plugins and skills work across your entire workspace — build a skill once (like a custom linter for your internal framework) and it's available in every project, every thread, every team member's session. Previously you had to copy `.amp/skills` folders between repos. Now you teach the agent once per organization.

**How does the Dial work with ChatGPT subscriptions?**
The Dial (low/medium/high/ultra modes) now detects what you already pay for. Link your ChatGPT subscription and Amp runs on it — no double-billing, no BYOK friction. You're not locked into Amp's token pricing; you bring your own model access. For teams on ChatGPT Enterprise or Plus, this removes unpredictable costs.

**Can AmpCode agents really coordinate with each other?**
Yes. Since July 14, agents can spawn sub-agents, exchange files, and coordinate across threads. One agent handles frontend, another backend, a third writes tests — they pass artifacts back and forth. You don't write YAML; you just say "have the frontend agent coordinate with the backend agent on the API contract."

**What's the deal with AmpCode's monthly subscriptions?**
Flat-rate monthly pricing launched July 17. No token counting, no surprise bills. If you want predictability over optimization, it's there alongside the Dial's effort-based routing.

**Is AmpCode fully local/air-gapped?**
No — this is still a cloud-dependent platform. No fully air-gapped mode exists yet. Some Orb sizes and GPU features are Linux-first (Windows parity is improving but not complete). For local-first workflows, consider OpenCode + Ollama or Hermes Agent instead.

**What happened when Amp swapped the default model overnight?**
On July 23, Amp swapped the default model and nobody complained. Because the Dial abstracts the model — you pick *effort level* (low/medium/high/ultra), not *model name*. Amp routes to whatever model delivers that effort at the best price/performance. This proves the future: you stop caring about model names and care about outcomes.
