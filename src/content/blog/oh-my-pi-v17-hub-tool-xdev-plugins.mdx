...
updatedDate: "2026-09-24T10:00:00Z"
...
- **New `omp compress` command.** Rewrites single or multiple text files (supporting glob patterns and concurrent processing) into dense "prompt registers" using an isolated two-tool agent loop. Think of it as a smart summarizer that turns verbose source files into compact context you can feed back to the agent — perfect for reducing token usage on large codebases. This is essentially automated context engineering — our [context engineering for coding agents](/blog/context-engineering-coding-agents-2026/) guide covers the manual version of this workflow.
...
## v18.3.0 — Tool Documentation, Interrupt Reasons, and Daily Polish (Sep 24)

The v18 series has arrived! **v18.3.0** (Sep 24) continues the daily release cadence with meaningful improvements:

### @oh-my-pi/pi-agent-core

**Added:**
- **AgentTool.docTopics support** — Agents can now document their tools on demand through the new `AgentTool.docTopics` method. This makes tool capabilities discoverable and self-documenting.
- **`TOOL_INTERRUPT_ABORT_REASON`** — Interruptible tools can now distinguish queued steering, peer messages, or background completions from a full run abort. This gives agents much finer-grained control over interruption handling.

### @oh-my-pi/pi-ai

**Fixed:**
- **Claude Opus 5.5 mid-session switch to high-effort reasoning** — Fixed an issue where switching to high-effort reasoning mid-session wasn't being applied correctly.

### v18.2.11 (Sep 23) — Background Job Fixes

- **Fixed background job completions interrupting foreground Bash and eval calls** — Previously, background job completions could cause foreground calls to be repeatedly moved into the background.
- **Fixed stream finalization when a provider ends without emitting a completion or error event** — Ensures the final assistant message is preserved and corresponding message lifecycle events are emitted.
- **Fixed tool execution being incorrectly skipped when host streams end** — Tools now execute reliably even when host stream handling has edge cases.

### v18.2.10 (Sep 22) — Stream Finalization and Tool Execution

- **Stream finalization fix** — When a provider ends without emitting a completion or error event, the final assistant message is now preserved with proper lifecycle events.
- **Tool execution skip fix** — Tools were incorrectly skipped when host streams ended; this is now resolved.
...
- `edit.enforceSeenLines` (defaults to `false`) is a new opt-in safety flag: when enabled, the agent refuses to edit lines that have not been fully displayed in the session. If self-review quality matters to you, enable it in settings. This aligns with the verification mindset in our [coding agent security checklist 2026](/blog/coding-agent-security-checklist-2026/).
...
Oh My Pi generally self-updates, but the official upgrade paths are: macOS/Linux via `curl -fsSL https://omp.sh/install | sh`, Windows via `irm https://omp.sh/install.ps1 | iex`, Homebrew users via `brew upgrade can1357/tap/omp`, and npm users via `bun update -g @oh-my-pi/pi-coding-agent`. Run the version check after updating to confirm you are on **v18.3.0**. If you are automating agents, watch for config settings referencing the old discovery or resolve paths and replace them per the v17.0.0 release notes.

## The bottom line

v17 was the biggest internal change to the Oh My Pi agent in months: one tool to rule messaging, background jobs, and long-running processes; transparent `xd://` approvals instead of hidden ones; and a serious plugins standard that makes extension safer. Now v18 arrives with tool documentation on demand, interrupt reason distinctions, and the same daily polish cadence. For beginners the practical wins keep stacking: the stability fixes in v17.2.12 (no more memory leaks, idle CPU spin, or dropped search sources), reasoning controls in v17.2.14, v17.2.15's `omp compress`, natural-language `omp cleanse`, MCP fixes, handoff artifact preservation, and hardened tar parsing, v17.3.2's TUI stall fix, hashline path fix, and Antigravity quota smarts, v17.3.3's Gemini reasoning-loop kill, malformed-call recovery, hashline edge-case normalization, and TUI markdown rendering fix, and now v18's AgentTool.docTopics, TOOL_INTERRUPT_ABORT_REASON, and background job fixes. If you skipped the v16 series updates, the jump to v18.3.0 is absolutely worth it.
