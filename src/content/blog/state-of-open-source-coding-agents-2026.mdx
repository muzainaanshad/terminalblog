...
There is no single best option — the right choice depends on your workflow. Hermes Agent is the most feature-complete (cron scheduling, multi-provider routing, persistent memory, orchestration). Oh My Pi is best for operators who want vision and browser automation. OpenCode is the simplest to audit and extend via SKILL.md files. Pi.dev excels at long-running background tasks with codebase memory. For most teams, the decision comes down to three questions: unattended execution, browser access, and model portability.

**Are open-source coding agents free?**
The software itself is free (MIT-licensed for most), but you pay for API tokens. An open-source agent like Hermes or OpenCode requires you to bring your own API keys (BYOK) for models like Claude, GPT, or Gemini. Monthly token costs range from $15-75 for light use to $750-1500 for heavy all-day coding, depending on which model you route to.

**How do open-source coding agents compare to Claude Code?**
Claude Code offers a polished, opinionated experience backed by Anthropic's Claude models and enterprise deployment (GCP Gateway, GitHub @claude). Open-source agents trade that polish for flexibility: swappable models, persistent memory, cron scheduling, and no vendor lock-in. Claude Code dominates on simplicity; open-source agents dominate on data sovereignty and customization.

**What is AGENTS.md and why does it matter for coding agents?**
AGENTS.md is a convention file placed in your repository root that tells any coding agent how your project is structured — build commands, test conventions, files to avoid, and security rules. It's the highest-leverage single file you can add: it turns a generic agent into one that respects your workflow. Most open-source agents (Hermes, OpenCode, Oh My Pi) read AGENTS.md natively.

**Are open-source coding agents secure?**
Open-source agents run with your credentials and filesystem access, so security depends on your setup. Best practices: scope API keys to least privilege, run in a sandbox where possible, gate destructive operations behind human approval, and review what subagents can access. Hermes's credential-guard system and Gitlawb Zero's local-only model represent two approaches to the security problem. See the Coding Agent Security Checklist for a full audit framework.
