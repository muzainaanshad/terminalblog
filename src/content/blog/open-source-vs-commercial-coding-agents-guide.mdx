...
**Q1: What is the main difference between open-source and commercial coding agents?**

Commercial agents (Claude Code, Cursor, Copilot CLI) provide a polished, turnkey experience with vendor support but lock you into their model, pricing, and roadmap. Open-source agents (Hermes, OpenCode, Goose) give you inspectable code, multi-provider flexibility, and self-hosting, but you own setup, maintenance, and security.

**Q2: Is open-source really free?**

The agent code is free, but you pay in setup time, API token costs, and operational burden (key rotation, plugin audits, on-call for agent failures). A free agent that nobody maintains is expensive in calendar time.

**Q3: What is a hybrid stack and why do most teams use one?**

A hybrid uses a commercial IDE agent (Cursor) for daily interactive speed and an open-source terminal agent (Hermes, OpenCode) for automation, cron jobs, and ownership-sensitive work. This matches each tool's strength and avoids single-vendor risk.

**Q4: How do I decide which path fits my team?**

Use the decision rubric in this guide: score your constraints (self-hosting, time-to-first-PR, unattended jobs, multi-provider, support needs, budget predictability, exit ability) 1-5. Highest weighted total wins the default; keep the other class as a specialist.

**Q5: What security questions should I ask regardless of which class I choose?**

Can we deny network for untrusted tools? Are secrets redacted in logs? Can we pin versions for a freeze window? Who can change production with the agent's credentials? How do we revoke a compromised plugin within an hour? See our [security checklist](/blog/coding-agent-security-checklist-2026/).
